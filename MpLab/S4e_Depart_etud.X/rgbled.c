...
{  
   static unsigned short sAccR = 0, sAccG = 0, sAccB = 0;
    ...
    lat_LED8_R = (sAccR & 0x100) ? 1: 0;    // si (sAccR & 0x100), si vrai alors lat_LED8_R = 1, si faux lat_LED8_R = 0
...
**      the pins corresponding to R, G and B colors are initialized as digital outputs and Timer 5 is configured.
**          
*/
void RGBLED_Init()
{
  ...
  //  RGBLED_Timer5Setup();
    ...
    T3CONbits.TCKPS = 3;                //            1:64 prescale value
   ...
   OC3CONbits.OCTSEL = 1;   // Timer2 is the clock source for this Output Compare module
   ...
   OC4CONbits.OCTSEL = 1;   // Timer2 is the clock source for this Output Compare module
   ...
   OC5CONbits.OCTSEL = 1;   // Timer2 is the clock source for this Output Compare module
...
{
    // stop the timer
      T3CONbits.ON = 0;   // turn off Timer5
    // turn off colors
    lat_LED8_R = 0;
    lat_LED8_G = 0;
    lat_LED8_B = 0;
