...
//#include "app_commands.h"
int8_t _TCPIP_PumpDNS(const char * hostname, IPV4_ADDR *ipv4Addr);
// *****************************************************************************
// *****************************************************************************
...
void TCPIP_Initialize ( void )
{
    ...
    appData.state = TCPIP_TCPIP_WAIT_INIT;
...
void TCPIP_Tasks ( void )
{
    ...
    {
        case TCPIP_TCPIP_WAIT_INIT:
            tcpipStat = TCPIP_STACK_Status(sysObj.tcpip);
            if(tcpipStat < 0)
            {   // some error occurred
                SYS_CONSOLE_MESSAGE(" APP: TCP/IP stack initialization failed!\r\n");
                appData.state = TCPIP_TCPIP_ERROR;
            }
            else if(tcpipStat == SYS_STATUS_READY)
            {
                ...
                appData.state = TCPIP_TCPIP_WAIT_FOR_IP;

            }
            break;


        case TCPIP_TCPIP_WAIT_FOR_IP:
			...
			appData.state = TCPIP_TCPIP_WAITING_FOR_COMMAND;
            SYS_CONSOLE_MESSAGE("Waiting for command type: sendudppacket\r\n");
            
			break;
        case TCPIP_TCPIP_WAITING_FOR_COMMAND:
        {
            SYS_CMD_READY_TO_READ();
            if (TCPIP_Send_Packet)
            {
                TCPIP_DNS_RESULT result;
                TCPIP_Send_Packet = false;
                result = TCPIP_DNS_Resolve(TCPIP_Hostname_Buffer, TCPIP_DNS_TYPE_A);
                if (result == TCPIP_DNS_RES_NAME_IS_IPADDRESS)
                {
                    IPV4_ADDR addr;
                    TCPIP_Helper_StringToIPAddress(TCPIP_Hostname_Buffer, &addr);
                    uint16_t port = atoi(TCPIP_Port_Buffer);
                    ...
                    appData.state = TCPIP_TCPIP_WAIT_FOR_CONNECTION;
                    break;
                }
                if (result < 0)
                {
                ...
                appData.state = TCPIP_TCPIP_WAIT_ON_DNS;
            }
        }
        break;

        /*case TCPIP_TCPIP_WAIT_ON_DNS:
        {
            IPV4_ADDR addr;
            switch (_TCPIP_PumpDNS(TCPIP_Hostname_Buffer, &addr))
            {
                case -1:
                {
        ...
        case TCPIP_TCPIP_WAIT_FOR_CONNECTION:
        {
            ...
            TCPIP_UDP_ArrayPut(appData.socket, (uint8_t*)TCPIP_Message_Buffer, strlen(TCPIP_Message_Buffer));
            TCPIP_UDP_Flush(appData.socket);
            appData.mTimeOut = SYS_TMR_SystemCountGet() + SYS_TMR_SystemCountFrequencyGet();
            appData.state = TCPIP_TCPIP_WAIT_FOR_RESPONSE;
        }
        break;

        case TCPIP_TCPIP_WAIT_FOR_RESPONSE:
        {
            char buffer[180];
