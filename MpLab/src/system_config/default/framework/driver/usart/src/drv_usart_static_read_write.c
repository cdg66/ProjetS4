...
size_t DRV_USART0_Read(void * destination,const size_t nBytes)
{
    ...
    {
        while((PLIB_USART_ReceiverDataIsAvailable(USART_ID_4)) && (count < nBytes))
        {
            ...
            data[count] = PLIB_USART_ReceiverByteReceive(USART_ID_4);
            count ++;
        }
    }
    return(count);
...
size_t DRV_USART0_Write(void * source, const size_t nBytes)
{
    ...
    {
        while((!PLIB_USART_TransmitterBufferIsFull(USART_ID_4)) && (count < nBytes))
        {
            /* This is not a blocking implementation. We write
               to the hardware till the FIFO is full. */
            PLIB_USART_TransmitterByteSend(USART_ID_4, data[count]);
            count ++;
